    ...
    constructor(props) {
        ...
        this.state = {
            ...
            getAjaxPageOtherParam:{
                parentCategoriesCode:this.props.router.params.parentCategoriesCode,
            },

            formDetailTitle: "商品子分类详情",
            showDetail: false,
                ...
                <span>
                  <a onClick={() =>{this.handleShowFromDataDetail(item.id,'edit')}}>编程</a>
                   ...
                   <a onClick={()=>{this.handleDel(item.id)}}>删除</a>
                      <span className="ant-divider" />
                  <Link to={{ pathname:`/productPage/${item.categoriesCode}`}}>产品</Link>
            </span>
            ),
        }];
                        ...
                        <Row gutter={16}>
                            ...
                            <Col className="gutter-row" span={4}>
                                <Button type="primary" onClick={() => this.setState({
                                    showFromAddAndEdit: true,
                                    formAddAndEditStatus: 'add',
                                    editInit: {parentCategoriesCode: this.props.router.params.parentCategoriesCode}
                                })}>{this.state.formAddTitle}</Button>
                            </Col>
        ...
        return (
            <Form onSubmit={this.handleSubmit}>
                <FormItem
                    ...
                    wrapperCol={{ span: pageConstant.inputWidth }}
                >
                    {getFieldDecorator('categoriesCode', {
                        rules: [{ required: true, message: '请输入商品子分类编码' }],
                    })(
                ...
                <FormItem
                    ...
                    wrapperCol={{ span: pageConstant.inputWidth }}
                >
                    {getFieldDecorator('categoriesName', {
                        rules: [{ required: true, message: '请输入商品子分类名称' }],
                    })(
                        <Input  autoComplete="off"/>
                    )}
                </FormItem>
...
const WrappedAddAndEditForm = Form.create({
    mapPropsToFields(props) {
        return {
            ...
            parentCategoriesCode:{
                value: props.editInit.parentCategoriesCode,
            }
        };
    },
})(AddAndEditForm);
